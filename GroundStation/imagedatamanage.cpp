...
void imagedatamanage::Result_Generate()
{
    //更新图像数组
    /*从imageTmpArray提取数据更新imageByteArray*/
    for(int i=0;i<Img_Size;i++)
    {
        ResultByteArray[i*3] = ResultByteArray[i*3+1] = ResultByteArray[i*3+2] = ResultByteArray[i];
    }

    //生成Qimage图像
    //图像数组数据保存到DisImage图像变量
    DisResult = QImage(ResultByteArray, Img_Width, Img_Height, QImage::Format_RGB888);

    //DisImage变换大小并存入imgScaled
    ResScaled = DisResult.scaled(400,240,Qt::KeepAspectRatio);

    emit Result_Ok_Signals();    //发出图像生成完毕信号
}
